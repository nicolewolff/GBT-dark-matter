...
def inject(xs, ys, theta, v_earth, sig_loc, sig_std, ann_cross_sec):
    sig_amp_ratio = 0
    if is_decay:
    ...
    sig_amp = sig_loc * amp_ratio
    shift_factor = get_shift_factor(theta, v_earth)
    sig_loc, sig_std, sig_amp = sig_loc*shift_factor, sig_std*shift_factor, amp/shift_factor
